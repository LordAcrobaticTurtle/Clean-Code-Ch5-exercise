...
{
for (int i = 0; i < TX_NUM_CHANNELS; i++) {m_tx.getEndPoints(i,&p_endPointsMin[i],&p_endPointsMax[i]);}
}

void RadioInterface::readSBUS() 
{
for (int ch = 0; ch < TX_NUM_CHANNELS; ch++) {m_prevChannelValues[ch] = m_channelValues[ch];}
